...
export default function NewReceipt() {
  ...
  useEffect(() => {
    const fetchVendors = async () => {
      try {
        ...
        // Vendors may not be accessible; keep form usable with manual entry.
  ...
  const onSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      const res = await purchaseReceipts.create(payload)
      const id = res.data.data?.receipt?.id || res.data.data?.grn?.id || res.data.data?.id
      if (id) navigate(`/purchases/receipts/${id}`)
      else navigate('/purchases/receipts')
    } catch (err) {
    ...
    <div className="space-y-4">
      <div className="flex items-center justify-between">
        <div>
          <div className="flex items-center gap-2">
            ...
            <h1 className="text-2xl font-semibold">New Receipt / GRN</h1>
          </div>
          <p className="text-[var(--text-secondary)]">Record goods received from a vendor</p>
        </div>
      </div>
      ...
      <form onSubmit={onSubmit} className="bg-white rounded-lg border border-[var(--border-strong)] p-6 space-y-4">
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
          <div>
            ...
            {vendors.length === 0 && (
              <p className="text-xs text-[var(--text-secondary)] mt-1">
                Vendor list not available. You can still submit if backend accepts vendor_id.
              </p>
            )}
          </div>
          ...
          <div>
            ...
            <input
              ...
              placeholder="Vendor challan / delivery note"
              className="w-full px-3 py-2 border border-[var(--border)] rounded-lg focus:outline-none focus:ring-2 focus:ring-[var(--primary)]"
            />
          </div>
        ...
        <div className="flex items-center justify-end gap-2 pt-2">
          <Link to="/purchases/receipts" className="px-4 py-2 rounded-lg border border-[var(--border)] hover:bg-gray-50">
            Cancel
          </Link>
          <button
            type="submit"
