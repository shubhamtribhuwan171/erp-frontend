      ...
      <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-4">
        {loading ? (
          ...
          <div className="col-span-full text-center py-8 text-[var(--secondary)]">
            No vendors found.{' '}
            <Link to="/purchases/vendors/new" className="text-[var(--primary)] hover:underline">
              Add one
            </Link>
          </div>
        ) : (
          vendors.map((vendor) => (
            ...
            >
              <div className="flex items-start justify-between">
                ...
                <span
                  className={`px-2 py-1 rounded-full text-xs ${vendor.status === 'active' ? 'bg-green-100 text-green-700' : 'bg-gray-100 text-gray-700'}`}
                >
                  {vendor.status}
                </span>
              </div>
              ...
              <div className="mt-4 pt-4 border-t border-[var(--border)] flex items-center justify-end gap-2">
                <Link to={`/purchases/vendors/${vendor.id}`} className="p-2 hover:bg-gray-100 rounded" title="View">
                  <Eye size={16} className="text-[var(--secondary)]" />
                </Link>
                <Link
                  to={`/purchases/vendors/${vendor.id}/edit`}
