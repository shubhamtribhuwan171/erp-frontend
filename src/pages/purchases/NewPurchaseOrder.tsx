...
import { inventory, purchases } from '../../lib/api'
...
export default function NewPurchaseOrder() {
  ...
  useEffect(() => {
    const run = async () => {
      ...
      setError('')
      try {
        const [vendorRes, itemsRes, whRes, unitRes] = await Promise.all([
  ...
  }, [vendorId, lines, submitting, loadingRefs])
  ...
  const handleCreate = async () => {
    ...
    try {
      ...
      const res = await purchases.orders.create(payload)
      const created = res.data.data ?? res.data
      navigate(`/purchases/orders/${created.id}`)
    } catch (e: any) {
      setError(e?.response?.data?.message ?? e?.message ?? 'Failed to create purchase order')
    } finally {
      setSubmitting(false)
    }
  }
      ...
      <div className="flex items-center justify-between">
        <div className="flex items-center gap-3">
          ...
          <div>
            ...
            <p className="text-[var(--text-secondary)]">Create a draft PO</p>
          </div>
        </div>
