...
import { sales, inventory } from '../../lib/api'
    ...
    <div className="bg-white rounded-lg border border-[var(--border)] p-5">
      <div className="flex items-start justify-between">
        <div>
          ...
          {change !== undefined && (
            <div
              className={`flex items-center gap-1 mt-2 text-sm ${change >= 0 ? 'text-[var(--success)]' : 'text-[var(--danger)]'}`}
            >
...
export default function Dashboard() {
  ...
  if (error) {
    if (isModuleDisabledError(error)) return <ModuleDisabledState moduleName="Dashboard" />
  ...
  return (
    <div className="space-y-6">
      ...
      <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-4">
        <KPICard title="Total Sales" value="₹4,52,300" change={12} icon={DollarSign} color="bg-blue-500" />
        <KPICard title="Pending Orders" value="12" icon={ShoppingCart} color="bg-orange-500" />
        <KPICard title="Total Customers" value="156" change={8} icon={Users} color="bg-green-500" />
        <KPICard title="Low Stock Items" value="5" icon={AlertTriangle} color="bg-red-500" />
      </div>
