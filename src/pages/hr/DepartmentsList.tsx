  ...
  return (
    <div className="space-y-4">
      <div className="flex items-center justify-between">
        <div><h1 className="text-2xl font-semibold">Departments</h1><p className="text-[var(--secondary)]">Organize employees</p></div>
        <button onClick={() => setShowForm(!showForm)} className="bg-[var(--primary)] text-white px-4 py-2 rounded-lg flex items-center gap-2"><Plus size={18} /> Add</button>
      </div>
      ...
      <div className="grid grid-cols-2 md:grid-cols-4 gap-4">
        {loading ? <div>Loading...</div> : depts.map(d => (
          <div key={d.id} className="bg-white rounded-lg border p-4 flex items-center gap-3">
            <div className="w-10 h-10 bg-gray-100 rounded-lg flex items-center justify-center"><FolderTree size={20} className="text-[var(--secondary)]" /></div>
            <div><p className="font-medium">{d.name}</p><p className="text-xs text-[var(--secondary)]">{d.code}</p></div>
          </div>
        ))}
      </div>
    </div>
