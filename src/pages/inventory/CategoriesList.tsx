  ...
  return (
    <div className="space-y-4">
      <div className="flex items-center justify-between">
        ...
        <button onClick={() => setShowForm(!showForm)} className="bg-[var(--primary)] text-white px-4 py-2 rounded-lg flex items-center gap-2">
      ...
      <div className="grid grid-cols-2 md:grid-cols-4 gap-4">
        {loading ? <div>Loading...</div> : categories.map(cat => (
          <div key={cat.id} className="bg-white rounded-lg border border-[var(--border)] p-4 flex items-center gap-3">
            <div className="w-10 h-10 bg-gray-100 rounded-lg flex items-center justify-center"><Folder size={20} className="text-[var(--secondary)]" /></div>
